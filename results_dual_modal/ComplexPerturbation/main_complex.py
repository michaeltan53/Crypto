...
class DualModalExperiment:
    ...
    def step3_analyze_and_visualize(self, all_errors, save_dir: str = "results_dual_modal"):
        ...
        ptype = ["KITTI", "DrivingStereo", "TCIA"]
        ptype_index = -1
        for sweep_results in all_errors:
            ...
            if ptype[ptype_index] == "complex":
                ...
                plt.plot(sweep_results['strengths'], sweep_results['ratio_errors'], label=ptype[ptype_index], linewidth=2)
                os.makedirs(save_dir, exist_ok=True)
                with open(ptype[ptype_index]+".txt", "w") as f:
                    for val in sweep_results['ratio_errors']:
                        f.write(f"{val}\n")
        ...
        plt.grid(True)
        plt.legend()
        plot_path = os.path.join(save_dir, "Complex+DrivingStereo.png")
        plt.savefig(plot_path)
        plt.close()

        # 生成并保存报告
        report = self.analyzer.generate_report()
        report_path = os.path.join(save_dir, "Complex+DrivingStereo.txt")
